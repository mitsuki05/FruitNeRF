...
@dataclass
class FruitNerfDataParserConfig(DataParserConfig):
    """Nerfstudio dataset config"""

    _target: Type = field(default_factory=lambda: FruitNerf)
...
@dataclass
class FruitNerf(DataParser):
    """Nerfstudio DatasetParser"""

    config: FruitNerfDataParserConfig
    downscale_factor: Optional[int] = None
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        for frame in meta["frames"]:
            ...
            fname = self._get_fname(filepath.as_posix().replace('\\', '/'), data_dir)
            ...
            if "semantic_path" in frame:
                semantic_filepath = Path(frame["semantic_path"]).as_posix().replace('\\', '/')
        ...
        if len(semantic_filenames) > 0:
            classes = ['apple', 'stuff']#meta["semantics"]
            colors = torch.zeros(classes.__len__())
            # Stuff
            colors[0] = 0\
            # Fruit Class
            colors[1] = 255
            colors /= 255.0
            semantics = Semantics(filenames=semantic_filenames, classes=classes, colors=colors, mask_classes=classes)
        ...
        dataparser_outputs = DataparserOutputs(
            ...
            metadata={"semantics": semantics} if len(semantic_filenames) > 0 else {},
        )
